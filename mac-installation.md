...
6.0.14](https://www.virtualbox.org/wiki/Download_Old_Builds_6_0#DownloadVirtualBoxOldBuilds:VirtualBox6.0:~:text=VirtualBox%206.0.14%20(released%20October%2015%202019)). This is the VM we will be using to run ACL2s. We require version is 6.0.14 because we have tested with this version; other versions may cause installation problems later. The latest version 6.1 is not supported by vagrant, which we need, so certainly do not use it. (N.B. On recent versions of OSX the installation may fail the first time you try it for lack of sufficient permission.)
  ...
  8. Place the following [Vagrantfile](https://www.khoury.northeastern.edu/~pete/courses/Logic-and-Computation/2020-Spring/Vagrantfile.txt) in that newly-created directory. In your `xterm`, rename the file to just `Vagrantfile`. We say to do this in an `xterm` to make sure-sure that you removed the `.txt` extension. 
