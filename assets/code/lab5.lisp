...
;; 1. Define and test a procedure var-occursp that takes a var and
;; an expr and returns t if that variable occurs (in the technical
;; sense of the term) in the expression, and nil otherwise. Do not use
;; helper procedures or change the method signature.
...
(check= (var-occursp 'x 'x) t)
(check= (var-occursp 'x ''x) nil)
(check= (var-occursp 'x '(let ((y 5)) y)) nil)
(check= (var-occursp 'y '(let ((x 5)) (* 'x y))) t)
(check= (var-occursp 'y '(let ((x 5))
			   (let ((y 7))
			     (* x y))))
	t)
(check= (var-occursp 'y '(let ((x 5))
...
(check= (var-occursp 'x '(let ((x 7))
			   (let ((x 5))
			     (let ((y z))
			       (* z y)))))
	nil)

(check= (var-occursp 'x '(let ((x 5))
			   (let ((x 6))
			     x)))
	t)
(check= (var-occursp 'y '(let ((y (let ((x 5))
				    '(* x y))))
			   (* x z)))
	nil)
(check= (var-occursp 'x '(let ((x (* x x)))
			   (* x x)))
	t)


;; 2. Define and test a procedure var-occurs-freep that takes a var
;; and an expr and returns t if that variable occurs free in the
;; expression, and nil otherwise. Do not use helper procedures or
;; change the method signature.

(definec var-occurs-freep (v :var e :expr) :bool
  ...
  )


(check= (var-occurs-freep 'x 'x) t)
(check= (var-occurs-freep 'x ''x) nil)
(check= (var-occurs-freep 'x '(let ((y 5)) y)) nil)
(check= (var-occurs-freep 'y '(let ((x 5)) (* 'x y))) t)
(check= (var-occurs-freep 'y '(let ((x 5))
				(let ((y 7))
				  (* x y))))
	nil)
(check= (var-occurs-freep 'y '(let ((x 5))
...
(check= (var-occurs-freep 'x '(let ((x 7))
				(let ((x 5))
				  (let ((y x))
				    (* x y)))))
	nil)

(check= (var-occurs-freep 'x '(let ((x 5))
				(let ((x 6))
				  x)))
	nil)
(check= (var-occurs-freep 'y '(let ((y (let ((x 5))
					 (* x y))))
				(* x y)))
	t)
(check= (var-occurs-freep 'x '(let ((x (* x x)))
				(* x x)))
	t)
...
In the second half of this lab, we will practice equational reasoning
